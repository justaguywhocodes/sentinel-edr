/*
 * akesoedr-agent/json_writer.cpp
...
std::string
JsonWriter::SerializeEvent(const AKESOEDR_EVENT& evt,
...
void
JsonWriter::WriteEvent(const AKESOEDR_EVENT& evt,
                       const std::wstring& parentImagePath)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string
JsonWriter::EventToJson(const AKESOEDR_EVENT& evt,
                        const std::wstring& parentImagePath)
{
    std::string json;
    json.reserve(1024);
...
std::string
JsonWriter::ProcessCtxToJson(const AKESOEDR_PROCESS_CTX& ctx,
                              const std::wstring& parentImagePath)
{
    std::string json;
    json.reserve(512);
...
std::string
JsonWriter::PayloadToJson(const AKESOEDR_EVENT& evt)
{
    switch (evt.Source) {
...
std::string
JsonWriter::HookPayloadToJson(const AKESOEDR_HOOK_EVENT& hook)
{
    std::string json;
    char numBuf[32];
     ...
     */
    if (hook.Function == AkesoEDRHookNtCreateNamedPipeFile) {
        json += ",\"pipeName\":\"";
...
std::string
JsonWriter::ProcessPayloadToJson(const AKESOEDR_PROCESS_EVENT& proc)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::ThreadPayloadToJson(const AKESOEDR_THREAD_EVENT& thread)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::ObjectPayloadToJson(const AKESOEDR_OBJECT_EVENT& obj)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::ImageLoadPayloadToJson(const AKESOEDR_IMAGELOAD_EVENT& img)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::RegistryPayloadToJson(const AKESOEDR_REGISTRY_EVENT& reg)
{
    std::string json;
...
std::string
JsonWriter::FilePayloadToJson(const AKESOEDR_FILE_EVENT& file)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::PipePayloadToJson(const AKESOEDR_PIPE_EVENT& pipe)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::NetworkPayloadToJson(const AKESOEDR_NETWORK_EVENT& net)
{
    std::string json;
    char numBuf[32];

    json += "{\"direction\":\"";
    json += (net.Direction == AkesoEDRNetOutbound) ? "Outbound" : "Inbound";
    json += "\"";
...
std::string
JsonWriter::AmsiPayloadToJson(const AKESOEDR_AMSI_EVENT& amsi)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::AlertPayloadToJson(const AKESOEDR_ALERT_EVENT& alert)
{
    std::string json;
...
std::string
JsonWriter::TamperPayloadToJson(const AKESOEDR_TAMPER_EVENT& tamper)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::EtwPayloadToJson(const AKESOEDR_ETW_EVENT& etw)
{
    std::string json;
    char numBuf[32];
...
std::string
JsonWriter::ScannerPayloadToJson(const AKESOEDR_SCANNER_EVENT& scan)
{
    std::string json;
    char numBuf[32];
