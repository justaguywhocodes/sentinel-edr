/*
 * akesoedr-agent/output/siem_writer.cpp
...
bool
SiemWriter::Init(const AkesoEDRConfig& cfg)
{
    ...
    /* Parse endpoint URL */
    if (!ParseEndpointUrl()) {
        std::printf("AkesoEDRAgent: SIEM writer: invalid endpoint URL: %s\n",
                    m_endpoint.c_str());
        m_enabled = false;
        return false;
    }
    ...
    if (!m_hSession) {
        std::printf("AkesoEDRAgent: SIEM writer: WinHttpOpen failed (%lu)\n",
                    GetLastError());
        m_enabled = false;
        return false;
    }
    ...
    std::printf("AkesoEDRAgent: SIEM writer started → %s "
                "(batch=%u, flush=%us)\n",
...
void
SiemWriter::Enqueue(const AKESOEDR_EVENT& evt,
...
SiemWriter::WorkerLoop()
{
    ...
    std::printf("AkesoEDRAgent: SIEM writer stopped\n");
}
...
SiemWriter::HttpPost(const std::string& ndjsonBatch)
{
    ...
    if (!hConnect) {
        std::printf("AkesoEDRAgent: SIEM: WinHttpConnect failed (%lu)\n",
                    GetLastError());
        return false;
    }
    ...
    if (!hRequest) {
        std::printf("AkesoEDRAgent: SIEM: WinHttpOpenRequest failed (%lu)\n",
                    GetLastError());
    ...
    if (ok) {
        ...
        if (ok) {
            ...
            if (statusCode >= 200 && statusCode < 300) {
                ...
                std::printf("AkesoEDRAgent: SIEM: HTTP %lu response\n",
                            statusCode);
            }
        }
    }
    ...
    if (!ok && !success) {
        std::printf("AkesoEDRAgent: SIEM: HTTP POST failed (%lu)\n",
                    GetLastError());
    }
...
SiemWriter::SpillToDisk(const std::string& ndjsonBatch)
{
    ...
    if (currentSize + ndjsonBatch.size() > m_spillMaxBytes) {
        std::printf("AkesoEDRAgent: SIEM: spill file full (%llu MB), "
                    "dropping %zu bytes\n",
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        std::printf("AkesoEDRAgent: SIEM: failed to open spill file (%lu)\n",
                    GetLastError());
        return;
    }
    ...
    std::printf("AkesoEDRAgent: SIEM: spilled %lu bytes to disk\n", written);
}
...
SiemWriter::DrainSpillFile()
{
    ...
    std::printf("AkesoEDRAgent: SIEM: drained spill file (%lld bytes)\n",
                (long long)fileSize);
