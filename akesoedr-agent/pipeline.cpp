/*
 * akesoedr-agent/pipeline.cpp
 * Event processing pipeline implementation.
 *
 * Four thread groups:
 *   1. Driver port receiver — connects to \AkesoEDRPort, receives
 *      AKESOEDR_FILTER_MSG via FilterGetMessage, pushes events to queue.
 *   2. Pipe server — listens on \\.\pipe\AkesoEDRTelemetry, accepts
...
void
EventQueue::Push(const AKESOEDR_EVENT& evt)
{
    {
...
bool
EventQueue::Pop(AKESOEDR_EVENT& evt, DWORD timeoutMs)
{
...
DriverPortReceiverThread()
{
    ...
    AgentLog("AkesoEDRAgent: Driver port receiver started\n");
    ...
    while (!g_Shutdown.load()) {
        ...
        if (FAILED(hr)) {
            AgentLog("AkesoEDRAgent: Driver port not available (0x%08lX), "
                     "retrying in %lums...\n", hr, backoffMs);
        ...
        AgentLog("AkesoEDRAgent: Connected to driver port\n");
        backoffMs = 1000;
        ...
        while (!g_Shutdown.load()) {
            ...
            if (FAILED(hr)) {
                ...
                AgentLog("AkesoEDRAgent: FilterGetMessage failed 0x%08lX\n", hr);
                break;  /* Reconnect */
            }
    ...
    AgentLog("AkesoEDRAgent: Driver port receiver stopped\n");
}
...
{
    BYTE    readBuf[sizeof(UINT32) + sizeof(AKESOEDR_IPC_EVENT_MSG) + sizeof(AKESOEDR_EVENT)];
    UINT32  sequenceNum = 0;

    AgentLog("AkesoEDRAgent: Pipe client connected (PID %lu)\n", clientPid);
    ...
    while (!g_Shutdown.load()) {
        ...
        if (!ReadFile(hPipe, readBuf, sizeof(readBuf), &bytesRead, nullptr)
            ...
            AgentLog("AkesoEDRAgent: Pipe read error %lu\n", err);
            break;
        }

        /* Check for disconnect message */
        if (bytesRead >= sizeof(UINT32) + sizeof(AKESOEDR_IPC_HEADER)) {
            const auto* hdr = reinterpret_cast<const AKESOEDR_IPC_HEADER*>(
                readBuf + sizeof(UINT32));
            if (hdr->Type == AkesoEDRMsgDisconnect) {
                AgentLog("AkesoEDRAgent: Client PID %lu sent disconnect\n",
                         clientPid);
                break;
            }
        }
        ...
        auto status = AkesoEDRIpcDeserializeEvent(
    ...
    AgentLog("AkesoEDRAgent: Pipe client PID %lu disconnected\n", clientPid);
}
...
PipeListenerThread()
{
    AgentLog("AkesoEDRAgent: Pipe server started on %ls\n",
    ...
    while (!g_Shutdown.load()) {
        ...
        if (hPipe == INVALID_HANDLE_VALUE) {
            AgentLog("AkesoEDRAgent: CreateNamedPipe failed %lu\n",
                     GetLastError());
        ...
        if (ReadFile(hPipe, handshakeBuf, sizeof(handshakeBuf),
            ...
            if (bytesRead >= sizeof(UINT32) + sizeof(AKESOEDR_IPC_HANDSHAKE)) {
                ...
                if (frameLen >= sizeof(AKESOEDR_IPC_HANDSHAKE)) {
                    ...
                    if (AkesoEDRIpcHeaderValidate(&hs->Header) == AkesoEDRSerializeOk
                        ...
                        if (AkesoEDRIpcWriteFrame(replyBuf, sizeof(replyBuf),
                                &reply, sizeof(reply),
                                &replyBytes) == AkesoEDRSerializeOk) {
                            DWORD written = 0;
        ...
        if (clientPid == 0) {
            ...
            DisconnectNamedPipe(hPipe);
            CloseHandle(hPipe);
            AgentLog("AkesoEDRAgent: Rejected pipe client (bad handshake)\n");
            continue;
        }
    ...
    AgentLog("AkesoEDRAgent: Pipe server stopped\n");
}
...
ProcessorThread()
{
    AgentLog("AkesoEDRAgent: Processing thread started\n");
    ...
    AgentLog("AkesoEDRAgent: Processing thread stopped (%llu events)\n",
...
void
PipelineStart(const AkesoEDRConfig& cfg)
{
    ...
    if (!g_EventProcessor.Init(cfg)) {
        AgentLog("AkesoEDRAgent: WARNING: Failed to open JSON log file\n");
    }

    AgentLog("AkesoEDRAgent: Pipeline starting...\n");

    /* Initialize and start ETW consumer (before other threads) */
    if (EtwConsumerInit()) {
        EtwConsumerStart();
    } else {
        AgentLog("AkesoEDRAgent: WARNING: ETW consumer init failed "
    ...
    if (!AmsiProviderRegister(cfg.amsiDllPath)) {
        AgentLog("AkesoEDRAgent: WARNING: AMSI provider registration failed "
...
PipelineStop()
{
    AgentLog("AkesoEDRAgent: Pipeline stopping...\n");
    ...
    AgentLog("AkesoEDRAgent: Pipeline stopped\n");
}
