/*
 * common/constants.h
 * Central constants for AkesoEDR — no magic numbers in any component.
 *
...
#ifndef AKESOEDR_CONSTANTS_H
#define AKESOEDR_CONSTANTS_H
...
#define AKESOEDR_VERSION            "1.0.0"
#define AKESOEDR_VERSION_MAJOR      1
#define AKESOEDR_VERSION_MINOR      0
#define AKESOEDR_VERSION_PATCH      0

/* ── Driver device names ─────────────────────────────────────────────────── */

/* NT device name (kernel namespace) */
#define AKESOEDR_DEVICE_NAME        L"\\Device\\AkesoEDRDrv"

/* Symbolic link (user-mode visible via \\.\AkesoEDRDrv) */
#define AKESOEDR_SYMLINK_NAME       L"\\DosDevices\\AkesoEDRDrv"

/* User-mode device path for CreateFile */
#define AKESOEDR_DEVICE_USER_PATH   L"\\\\.\\AkesoEDRDrv"
...
#define AKESOEDR_IOCTL_BASE         0x800

/* CLI → Agent → Driver: query status */
#define IOCTL_AKESOEDR_STATUS       CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 0,      \
                                    METHOD_BUFFERED,              \
                                    FILE_ANY_ACCESS)

/* CLI → Agent: trigger on-demand scan */
#define IOCTL_AKESOEDR_SCAN         CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 1,      \
                                    METHOD_BUFFERED,              \
                                    FILE_READ_ACCESS | FILE_WRITE_ACCESS)

/* CLI → Agent: reload detection rules */
#define IOCTL_AKESOEDR_RULES_RELOAD CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 2,      \
                                    METHOD_BUFFERED,              \
                                    FILE_READ_ACCESS | FILE_WRITE_ACCESS)

/* CLI → Agent: get recent alerts */
#define IOCTL_AKESOEDR_ALERTS       CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 3,      \
                                    METHOD_BUFFERED,              \
                                    FILE_ANY_ACCESS)

/* CLI → Agent: get connection table */
#define IOCTL_AKESOEDR_CONNECTIONS  CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 4,      \
                                    METHOD_BUFFERED,              \
                                    FILE_ANY_ACCESS)

/* CLI → Agent: get tracked processes */
#define IOCTL_AKESOEDR_PROCESSES    CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 5,      \
                                    METHOD_BUFFERED,              \
                                    FILE_ANY_ACCESS)

/* CLI → Agent: get hook status */
#define IOCTL_AKESOEDR_HOOKS        CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 6,      \
                                    METHOD_BUFFERED,              \
                                    FILE_ANY_ACCESS)

/* Agent → Driver: enable/disable sensors */
#define IOCTL_AKESOEDR_SENSOR_CTL   CTL_CODE(FILE_DEVICE_UNKNOWN, \
                                    AKESOEDR_IOCTL_BASE + 7,      \
                                    METHOD_BUFFERED,              \
                                    FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
#define AKESOEDR_TAG_GENERAL        'cPnS'  /* SnPc — general allocations */
#define AKESOEDR_TAG_EVENT          'vEnS'  /* SnEv — event buffers */
#define AKESOEDR_TAG_PROCESS        'rPnS'  /* SnPr — process table entries */
#define AKESOEDR_TAG_REGISTRY       'gRnS'  /* SnRg — registry callback data */
#define AKESOEDR_TAG_IMAGE          'mInS'  /* SnIm — image-load data */
#define AKESOEDR_TAG_OBJECT         'bOnS'  /* SnOb — object callback data */
#define AKESOEDR_TAG_FILE           'lFnS'  /* SnFl — minifilter data */
#define AKESOEDR_TAG_NETWORK        'tNnS'  /* SnNt — WFP callout data */
#define AKESOEDR_TAG_STRING         'tSnS'  /* SnSt — string buffers */
#define AKESOEDR_TAG_HASH           'sHnS'  /* SnHs — hash computation */
#define AKESOEDR_TAG_KAPC           'AnS\0' /* SnA  — KAPC injection */
...
#define AKESOEDR_MINIFILTER_ALTITUDE    L"321000"
 ...
 */
#define AKESOEDR_OB_ALTITUDE        L"321000"

/* ── Protected process list (object callbacks, Phase 2) ──────────────────── */

#define AKESOEDR_PROTECTED_PROC_COUNT   3

#ifdef _KERNEL_MODE
/* Kernel-mode: use static array in the .c file that includes this */
#else
static const WCHAR* AKESOEDR_PROTECTED_PROCESSES[] = {
    L"lsass.exe",
    L"csrss.exe",
    L"services.exe"
};
 ...
 * monitoring to reduce noise. Configurable at runtime via akesoedr.conf.
 */
#define AKESOEDR_FS_EXCLUDE_COUNT       3

#ifdef _KERNEL_MODE
/* Kernel-mode: define in the .c file */
#else
static const WCHAR* AKESOEDR_FS_EXCLUSIONS[] = {
    L"\\Windows\\",
    L"\\Program Files\\",
    L"\\Program Files (x86)\\"
};
...
#define AKESOEDR_REG_EXCLUDE_COUNT      2

#ifdef _KERNEL_MODE
/* Kernel-mode: define in the .c file */
#else
static const WCHAR* AKESOEDR_REG_EXCLUSIONS[] = {
...
#define AKESOEDR_NET_MAX_EVENTS_PER_SEC     100     /* Per PID */

/* ── Scanner limits ──────────────────────────────────────────────────────── */

#define AKESOEDR_SCAN_MAX_FILE_SIZE         (50 * 1024 * 1024)  /* 50 MB */
#define AKESOEDR_SCAN_MAX_REGION_SIZE       (10 * 1024 * 1024)  /* 10 MB per memory region */
#define AKESOEDR_SCAN_CACHE_TTL_SEC         300                 /* 5 minutes */

/* ── Log rotation ────────────────────────────────────────────────────────── */

#define AKESOEDR_LOG_MAX_SIZE_BYTES         (100 * 1024 * 1024) /* 100 MB */

/* ── Hook DLL ring buffer ────────────────────────────────────────────────── */

#define AKESOEDR_HOOK_RING_BUFFER_SIZE      1000    /* Max buffered events */

/* ── Suspicious named pipes (Cobalt Strike defaults, Phase 5) ────────────── */

#define AKESOEDR_SUSPICIOUS_PIPE_COUNT      5

#ifndef _KERNEL_MODE
static const WCHAR* AKESOEDR_SUSPICIOUS_PIPES[] = {
    L"\\MSSE-",
    L"\\msagent_",
    L"\\postex_",
    L"\\status_",
...
/*
 * AkesoEDR custom AMSI provider — registered via IAntimalwareProvider.
 * Windows loads this COM in-process server (akesoedr-amsi.dll) into any
 * AMSI-instrumented application (PowerShell, VBScript, JScript).
 * {A3F5C8D2-7B1E-4A9F-8C6D-E5B2F1A47390}
 */
DEFINE_GUID(AKESOEDR_AMSI_PROVIDER_CLSID,
