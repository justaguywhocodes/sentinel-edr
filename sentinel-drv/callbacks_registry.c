/*
 * sentinel-drv/callbacks_registry.c
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, SentinelRegistryCallbackInit)
#pragma alloc_text(PAGE, SentinelRegistryCallbackStop)
#endif
...
static BOOLEAN
SentinelRegistryIsNoisy(
    _In_ PCUNICODE_STRING KeyPath
);

static VOID
SentinelFillProcessCtxForRegistry(
    _Out_ SENTINEL_PROCESS_CTX* Ctx,
    _In_  HANDLE                ProcessId
);

static VOID
SentinelEmitRegistryEvent(
    _In_     SENTINEL_REG_OP    Operation,
    _In_     PCUNICODE_STRING   KeyPath,
...
)
{
    ...
    case RegNtPostCreateKeyEx: {
        ...
        if (!SentinelRegistryIsNoisy(objectName)) {
            ...
            if (preInfo && preInfo->CompleteName) {
                SentinelEmitRegistryEvent(
                    SentinelRegOpCreateKey, objectName,
                    NULL, 0, NULL, 0);
            }
        }
    ...
    case RegNtPostSetValueKey: {
        ...
        if (!SentinelRegistryIsNoisy(objectName)) {
            ...
            if (dataSize > SENTINEL_MAX_REG_DATA) {
                dataSize = SENTINEL_MAX_REG_DATA;
            }

            SentinelEmitRegistryEvent(
                SentinelRegOpSetValue, objectName,
                setInfo->ValueName,
                setInfo->Type,
                setInfo->Data,
                dataSize);
    ...
    case RegNtPreDeleteValueKey: {
        ...
        if (!SentinelRegistryIsNoisy(objectName)) {
            SentinelEmitRegistryEvent(
                SentinelRegOpDeleteValue, objectName,
                delInfo->ValueName,
                0, NULL, 0);
        }
    ...
    case RegNtPreDeleteKey: {
        ...
        if (!SentinelRegistryIsNoisy(objectName)) {
            SentinelEmitRegistryEvent(
                SentinelRegOpDeleteKey, objectName,
                NULL, 0, NULL, 0);
        }
    ...
    case RegNtPreRenameKey: {
        ...
        if (!SentinelRegistryIsNoisy(objectName)) {
            SentinelEmitRegistryEvent(
                SentinelRegOpRenameKey, objectName,
                renameInfo->NewName,
                0, NULL, 0);
        }
...
)
{
    ...
    __try {
        ...
        if (Data && DataSize > 0) {
            ...
            if (copySize > SENTINEL_MAX_REG_DATA) {
                copySize = SENTINEL_MAX_REG_DATA;
            }
